...
const addBook = (request, h) => {
  ...
  if (pageCount < readPage) {
    const response = h.response({
      status: 'fail',
      message: 'Gagal menambahkan buku, readPage tidak boleh > pageCount'
    })
    response.code(400)
...
const getAllBook = (request, h) => {
  const { name, read, finished } = request.query
  ...
  if (read !== undefined) {
    filtered = filtered.filter((book) => book.read === !!Number(read))
  }
